...
class Trie:
    ...
    def recursiveSearchWithWildCard(self, node, word, index):
        if index==len(word): 
            return node.flag
        if word[index] in node.links: 
            return self.recursiveSearchWithWildCard(node.links[word[index]], word, index+1)
        return any(self.recursiveSearchWithWildCard(node.links[ch], word, index+1) for ch in node.links.keys()) if word[index]=="." else False
